...
toPWMRadBig = @(x) x/(pi*+100/200 )*0.5 + 0.5; 
...
L3=12;
L=6;
s = 'Tz(L)Rz(q1) Rx(q2) Tz(L1) Rx(q3) Tz(L2) Rz(q4) Rx(q5)';
dh = DHFactor(s);
dh.display
...
%%
%q = positions;
q=[0,0.977384381116825,1.256637061435917,0,-0.900590941226625];
T=robot.fkine(q);
...
%robot.plot(qs)
   ...
   T3=T2;
T3.t(1) = T2.t(1)+4;
qs2=robot.ikine(T3,'mask',[1 1 1 1 1 0]);
%robot.plot(qs)
...
T4=T3;
T4.t(2) = T4.t(2)+5
qs3=robot.ikine(T4,'mask',[1 1 1 1 1 0])
%robot.plot(qs)
...
T5=T4;
T5.t(2) = T5.t(2)+5
qs4=robot.ikine(T5,'mask',[1 1 1 1 1 0])
%robot.plot(qs)
